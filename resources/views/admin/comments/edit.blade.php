            ...
            <div class="row justify-content-center">
                <div class="col-md-8">
                    <div class="card shadow mt-5">
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.comments.update', $data->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="row my-3">
                                    <div class="col-md-3">
                                    ...
                                    <div class="col-md-6">
                                        <select name="user_id" id="user_id" class="form-control">
                                            @foreach ($users as $user)
                                                <option {{ $data->user_id == $user->id ? 'selected' : '' }} value="{{ $user->id }}">{{ $user->name . ' - ' . $user->email }}</option>
                                            @endforeach
                                        </select>
                                        @error('user_id')
                                        <span class="text-danger">{{ $message }}</span>
                                ...
                                <div class="my-3">
                                    ...
                                    <a href="{{ route('admin.comments.index') }}" class="me-2 btn btn-secondary float-end">Cancel</a>
                                </div>
                            </form>
                        </div>
                    </div>
